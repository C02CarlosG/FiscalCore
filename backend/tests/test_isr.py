...
def test_caso_a_enero_sin_pagos_previos():
    ...
    res = isr_provisional(
        ...
        mes=1, cu=CU, tasa=TASA, ptu=CERO, perdidas=CERO, retenciones_por_mes={},
    )
...
def test_caso_a_febrero_resta_pago_previo():
    ...
    res = isr_provisional(
        ...
        mes=2, cu=CU, tasa=TASA, ptu=CERO, perdidas=CERO, retenciones_por_mes={},
    )
...
def test_caso_a_marzo_continuidad_del_acumulado():
    ...
    res = isr_provisional(
        ...
        mes=3, cu=CU, tasa=TASA, ptu=CERO, perdidas=CERO, retenciones_por_mes={},
    )
...
def test_caso_b_retencion_de_isr_se_resta():
    ...
    res = isr_provisional(
        ...
        retenciones_por_mes={1: Decimal("1500")},
    )
...
def test_pago_negativo_se_trunca_a_cero():
    ...
    res = isr_provisional(
        ingresos_por_mes={1: Decimal("1000000")},
        mes=1, cu=CU, tasa=TASA, ptu=CERO, perdidas=CERO,
        retenciones_por_mes={1: Decimal("999999")},
    )
    assert res["pago_del_mes"] == Decimal("0.00")


def test_retencion_de_mes_anterior_se_refleja_en_pagos_previos():
    """Regresión (hallazgo de Kilo Code Review en PR #4): si un mes anterior del
    ejercicio tuvo retención de ISR, el pago real de ese mes fue MENOR al ISR
    acumulado bruto (se le restó la retención). `pagos_previos` del mes actual
    debe reflejar ese pago real, no el ISR acumulado bruto del mes anterior
    ignorando su retención — de lo contrario el pago del mes actual sale
    subestimado (se resta de más).
    ...
    res = isr_provisional(
        ingresos_por_mes={1: Decimal("1000000"), 2: Decimal("2200000")},
        mes=2, cu=CU, tasa=TASA, ptu=CERO, perdidas=CERO,
